...
export default ({test, assert}) => {
  ...
  test('Subscription - successful subscribe and receive update from server', async () => {
    ...
    // THEN: Подписка зарегистрирована
    await waitForCondition(
      () => Subscription._getSubscriptionCount() > 0,
      { timeout: 3000, message: 'Subscription should be registered' }
    );

    // Wait for potential update (optional - server may or may not send immediately)
    await timeout(2000);

    // 2. Callback был вызван с корректными данными (если сервер отправил обновление)
    if (receivedUpdate) {
  ...
  test('Subscription - unsubscribe stops receiving updates', async () => {
    ...
    await waitForCondition(
      () => Subscription._getSubscriptionCount() > 0,
      { timeout: 2000, message: 'Subscription should be registered' }
    );
    ...
    // THEN: Новые обновления не приходят
