...
export default ({ test, assert }) => {
  ...
  test('html() - tagged template function escapes values', () => {
    const name = '<script>alert("xss")</script>';
    const result = html`<div>${name}</div>`;

    assert(result.includes('&lt;script&gt;'), 'Should escape HTML');
    assert(!result.includes('<script>'), 'Should not include raw script tag');
  });

  test('html() - preserves already marked safe values', () => {
  ...
  test('html() - handles arrays', () => {
    const items = ['<b>one</b>', '<b>two</b>'];
    const result = html`<div>${items}</div>`;

    assert(result.includes('&lt;b&gt;'), 'Should escape array items');
  });
  ...
  test('safe() - escapes dangerous characters', () => {
    const dangerous = '<script>alert("xss")</script>';
    const result = safe(dangerous);

    assert(result === '&lt;script&gt;alert(&quot;xss&quot;)&lt;&#x2F;script&gt;', 'Should escape all dangerous chars');
  });

  test('safe() - removes reactive expressions', () => {
    const text = 'Hello {this.name} world';
    const result = safe(text);

    assert(result === 'Hello  world', 'Should remove {expressions}');
  });

  test('safe() - handles arrays recursively', () => {
    const arr = ['<div>', '<span>'];
    const result = safe(arr);

    assert(Array.isArray(result), 'Should return array');
    assert(result[0] === '&lt;div&gt;', 'Should escape first item');
    assert(result[1] === '&lt;span&gt;', 'Should escape second item');
  });
  ...
  test('Component - safe() function strips expressions', () => {
    const result = safe('<div>Hello {this.name}</div>');
    // safe() does HTML escaping AND strips expressions
    assert(result === '&lt;div&gt;Hello &lt;&#x2F;div&gt;', 'Should escape HTML and strip expressions');

    const result2 = safe(['class1 {dynamic}', 'class2']);
    assert(Array.isArray(result2), 'Should return array for array input');
    assert(result2[0] === 'class1 ', 'Should strip expressions from array items');
    assert(result2[1] === 'class2', 'Should keep non-expression items');
  });

  test('Component - raw() tagged template', () => {
    const name = 'World';
    const result = raw`Hello ${name}!`;

    // raw() returns marker + string
    assert(typeof result === 'string', 'Should return string');
    assert(result.includes('Hello World!'), 'Should include interpolated value');
  });

  test('Component - raw() with array values', () => {
    const classes = ['class1', 'class2'];
    const result = raw`<div class="${classes}"></div>`;

    assert(typeof result === 'string', 'Should return string');
    assert(result.includes('class1 class2'), 'Should join array with spaces');
  });
